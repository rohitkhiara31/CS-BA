...
def cos(l1,l2):
    # vectorizer=CountVectorizer().fit_transform([l1,l2])
    # vectors=vectorizer.toarray() 
   # csim=cosine_similarity([l1],[l2])
...
def similiarity(matrix, title, threshold, sparse ):  
    ...
    for i in range(len(title)):
           for j in range(i):
              if matrix[i][j]==1:  
                   v1,v2= buildVector(sparse[i], sparse[j])

                   if ((1-cluster.util.cosine_distance(v1,v2))>threshold):
...
num_perm=100
split = 1624 #63%
cos_threshold= [  list(range(6))[i]*.1 for i in range(6)]
...
for i in range(num_bootstrap):
    ...
    for cos_value in  cos_threshold:
          ...
          train_pairs= similiarity(  train_candidatePairs,   train_titleString,  cos_value,   train_sparseMatrix)
          train_accuracy=accuracy(  train_REALPAIRS,  train_pairs)
        
          print(cos_value,  train_accuracy)
    ...
    best_cos=cos_threshold[ f1.index(max( f1))]
    test_cos.append(best_cos)
    ...
    test_pairs= similiarity(test_candidatePairs,test_titleString, best_cos,test_sparseMatrix)
...
for num in num_bands:
     ...
     pairs= similiarity( candidatePairs,titleList, average_cos,sparseMatrix1)
